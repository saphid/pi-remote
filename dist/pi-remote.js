...
function parseShellWords(value, source) {
    const words = [];
    let current = '';
    let quote = '';
    let escaping = false;
    ...
    if (escaping)
        current += '\\';
    if (quote)
        fail(`unterminated quote in ${source}`);
    push();
    return words;
}
function configArgs(key, envName) {
    const raw = env(envName) ?? configGet(key, '');
    return raw ? parseShellWords(raw, `${envName}/${key}`) : [];
}
function configuredAgentArgs(agent) {
...
async function runServer(args) {
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        switch (arg) {
            ...
            case '--agent-switch':
            case '--agent-arg':
                options.agentSwitches.push(args[++index] ?? fail(`${arg} requires a value`));
                break;
            case '--project-root':
            ...
            default:
                ...
                else if (arg.startsWith('--agent-switch='))
                    options.agentSwitches.push(arg.slice('--agent-switch='.length));
                else if (arg.startsWith('--agent-arg='))
                    options.agentSwitches.push(arg.slice('--agent-arg='.length));
...
function firstUnexpectedPositional(args) {
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (!arg.startsWith('-'))
            return arg;
        if (['--host', '--project', '--new', '--sessions', '--saved-agent', '--saved-session-limit', '--session', '--agent', '--command', '--agent-switch', '--agent-arg', '--project-root', '--pi-bin'].includes(arg))
            index += 1;
    }
    return '';
}
...
function argsForInvocation(args) {
    if (invocationName() !== 'claude-remote')
        return args;
    const injected = ['--agent', 'claude'];
    if (!args.includes(CLAUDE_REMOTE_DEFAULT_SWITCH))
        injected.push('--agent-switch', CLAUDE_REMOTE_DEFAULT_SWITCH);
    if (args[0] === '--server')
        return ['--server', ...injected, ...args.slice(1)];
